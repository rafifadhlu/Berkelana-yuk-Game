...
public class TCameraPlayer : MonoBehaviour
{
    public float dirX;
    public float dirY;
    ...
    private void HandleInput()
    {
        for (int i = 0; i < Input.touchCount; i++)
        {
            ...
            if (touch.phase == TouchPhase.Moved || touch.phase == TouchPhase.Stationary)
            {
                ...
                if (IsTouchArea(touchPosition, rightHalf))
                {
                    Debug.Log("Area Touched");
    ...
    private bool IsTouchArea(Vector2 position, RectTransform area)
    {
        Vector2 localPoint;
